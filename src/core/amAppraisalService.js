...
const { getAllAmItemKeys } = require('./amAppraisalTemplate');

async function ensureTables() {
  await query(
    `CREATE TABLE IF NOT EXISTS am_appraisals (
      ...
      UNIQUE KEY uniq_am_appraisal_employee (employee_code),
      INDEX idx_am_appraisals_manager (manager_code),
      INDEX idx_am_appraisals_status (status)
...
function normalizeRatings(body, prefix) {
  const ratings = {};
  for (const key of getAllAmItemKeys()) {
...
async function deleteById(id) {
  ...
  await query(`DELETE FROM am_appraisals WHERE id=?`, [parseInt(id, 10) || 0], 'commit');
  return report;
}
