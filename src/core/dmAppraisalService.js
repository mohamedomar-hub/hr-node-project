...
const { getAllDmItemKeys } = require('./dmAppraisalTemplate');

async function ensureTables() {
  await query(
    `CREATE TABLE IF NOT EXISTS dm_appraisals (
      ...
      UNIQUE KEY uniq_dm_appraisal_employee (employee_code),
      INDEX idx_dm_appraisals_manager (manager_code),
      INDEX idx_dm_appraisals_status (status)
...
function normalizeRatings(body, prefix) {
  const ratings = {};
  for (const key of getAllDmItemKeys()) {
    const raw = body[`${prefix}_${key}`];
    ratings[key] = raw === undefined || raw === null ? '' : String(raw).trim();
  }
  return ratings;
}
...
async function deleteById(id) {
  ...
  await query(`DELETE FROM dm_appraisals WHERE id=?`, [parseInt(id, 10) || 0], 'commit');
  return report;
}
